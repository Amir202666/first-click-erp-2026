...
import type { Vendor, Item, Branch, Warehouse, PurchaseRequest, ItemUnitOption, Currency } from '../../types'
...
export default function EditPurchaseRequest() {
  ...
  const { data: pr, isLoading } = useQuery<PurchaseRequest>({
    queryKey: ['purchase-request', tenantId, id],
    queryFn: () => fetchPurchaseRequest(tenantId, Number(id)),
    enabled: !!tenantId && !!id,
  })
  ...
  useEffect(() => {
    if (!pr) return
    setDate(pr.date?.slice?.(0, 10) ?? '')
    setVendorId(pr.vendor_id ?? null)
    setBranchId(pr.branch_id ?? null)
    setWarehouseId(pr.warehouse_id ?? null)
    setReferenceNumber(pr.reference_number ?? '')
    setDiscountType('amount')
    setDiscountInputStr(pr.discount_amount != null ? String(pr.discount_amount) : '')
    setNotes(pr.notes ?? '')
    if (pr.lines?.length) {
  ...
  const updateMut = useMutation({
    mutationFn: (data: Record<string, unknown>) => updatePurchaseRequest(tenantId, Number(id), data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['purchase-requests'] })
      queryClient.invalidateQueries({ queryKey: ['purchase-request', tenantId, id] })
    ...
    <div className="p-4 space-y-4">
      ...
      {showSaveConfirm && (
        <ConfirmDialog
          ...
          message={lang === 'ar'
            ? 'هل تريد حفظ التعديلات؟ سيتم نقلك إلى نموذج طلب شراء جديد.'
            : 'Do you want to save the changes? You will be taken to a new purchase request form.'}
          variant="warning"
          isLoading={updateMut.isPending}
          confirmLabel={t.save}
          onConfirm={() => updateMut.mutate(buildPayload())}
