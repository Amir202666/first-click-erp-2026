...
import { fetchPayments, fetchSettings, fetchPaymentMethods, fetchAccounts, fetchTenantUsers } from '../../api/tenant'
import type { Payment, Account, PaymentMethod, PaginatedResponse } from '../../types'
import { formatDisplayDate, toLocalDateString, getDefaultDateRange, getReportPeriodRange, type ReportPeriodKey } from '../../utils/date'
import { formatAmount } from '../../utils/currency'
import { sortUsersForFilter } from '../../utils/tenantUsersForFilter'
import { paymentMethodLabel, createdByDisplayName } from '../../utils/paymentApiDisplay'
...
type ReceiptsReportColumnKey = 'number' | 'date' | 'invoiceNumber' | 'posted' | 'status' | 'branch' | 'amount' | 'paymentMethod' | 'customer' | 'reference' | 'employee'
const RECEIPTS_REPORT_COLUMN_KEYS: ReceiptsReportColumnKey[] = ['number', 'date', 'invoiceNumber', 'posted', 'status', 'branch', 'amount', 'paymentMethod', 'customer', 'reference', 'employee']
const RECEIPTS_COLUMNS_STORAGE_KEY = 'receiptsReportVisibleColumns'
...
export default function ReceiptsReport() {
  ...
  const [visibleColumns, setVisibleColumns] = usePersistedColumnVisibility(
    RECEIPTS_COLUMNS_STORAGE_KEY,
    RECEIPTS_REPORT_COLUMN_KEYS,
  )
  ...
  const columnLabels: Record<ReceiptsReportColumnKey, string> = {
    number: t.payments.voucherNumber,
    date: t.date,
    invoiceNumber: t.payments.associatedInvoiceNumber ?? 'رقم الفاتورة المرتبطة',
    posted: t.payments.posted ?? 'مرحّل',
    status: t.status,
    branch: t.journal.branch,
    amount: t.amount,
    paymentMethod: t.payments.paymentMethod,
    customer: t.journal.customerOrVendor,
    reference: t.payments.reference,
    employee: t.payments.employeeFilter ?? 'الموظف',
  }
  const visibleColumnKeys = RECEIPTS_REPORT_COLUMN_KEYS.filter((k) => visibleColumns[k])
  ...
  /** صف مرن: عرض ثابت لبعض الحقول + حقول تمتد لباقي المساحة */
  ...
  /** مثل خلية «عدد السجلات» في شبكة فلاتر الفواتير */
  const filterCellPageSize = 'min-w-0 w-full shrink-0 basis-[min(100%,220px)] max-w-[240px]'

  const filterParams = useMemo(() => {
    const p: Record<string, string> = { type: 'receipt', from_date: dateFrom, to_date: dateTo, per_page: String(perPage), page: String(page) }
    ...
    return p
  }, [dateFrom, dateTo, statusFilter, paymentMethodIdFilter, counterpartAccountIdFilter, cashBankAccountIdFilter, createdByFilter, numberFilter, invoiceIdFilter, perPage, page])

  const { data, isLoading } = useQuery<PaginatedResponse<Payment>>({
    queryKey: ['payments', tenantId, 'receipts-report', filterParams, perPage],
  ...
  const sortColumns = useMemo(
    () => [
      ...
      { key: 'invoiceNumber' as const, type: 'string' as const, getValue: (p: Payment) => p.invoice?.number ?? String(p.invoice_id ?? '') },
      {
        key: 'posted' as const,
      ...
      { key: 'branch' as const, type: 'string' as const, getValue: (p: Payment) => p.branch?.name ?? '' },
      ...
      { key: 'customer' as const, type: 'string' as const, getValue: (p: Payment) => p.customer?.name ?? '' },
      { key: 'reference' as const, type: 'string' as const, getValue: (p: Payment) => p.reference ?? '' },
      { key: 'employee' as const, type: 'string' as const, getValue: (p: Payment) => createdByDisplayName(p, '') },
    ],
    [lang],
  )
  const { sort, toggleSort, sortedRows } = useClientSort<Payment, ReceiptsReportColumnKey>(payments, sortColumns, { locale })
  ...
  const reportTitle = lang === 'ar' ? 'تقرير التحصيلات' : 'Receipts Report'
  const companyLogo = (settings as Record<string, unknown>)?.company_logo as string | undefined

  /** نفس ترتيب وعناوين خيارات «الفترة» في فواتير المبيعات */
  ...
  function onReceiptDateFromChange(value: string) {
    setDateFrom(value)
    setPage(1)
  }

  function onReceiptDateToChange(value: string) {
    setDateTo(value)
    setPage(1)
  }
  ...
  function buildPrintContent() {
    ...
    const rows = sortedRows.map((p) => {
      ...
      const empName = createdByDisplayName(p, '—')
      ...
      keys.forEach((k) => {
        ...
        else if (k === 'customer') cells.push(`<td>${p.customer?.name ?? '—'}</td>`)
        else if (k === 'reference') cells.push(`<td>${p.reference ?? '—'}</td>`)
        else if (k === 'employee') cells.push(`<td>${empName}</td>`)
      })
    ...
    if (amountIdx >= 0) {
      const amountCell = `<td class="num"><strong>${fmt(totals.amount)}</strong></td>`
      const countCell = `<td><strong>${totals.count}</strong></td>`
      footerRows = `<tr class="footer"><td colspan="${leftSpan}"><strong>${t.payments.totalAmount} / ${t.payments.reportTotal ?? 'إجمالي التقرير'}</strong></td>${amountCell}${rightSpan > 0 ? `<td colspan="${rightSpan}"></td>` : ''}</tr>
        <tr class="footer"><td colspan="${leftSpan}"><strong>${t.payments.totalCount}</strong></td>${countCell}${rightSpan > 0 ? `<td colspan="${rightSpan}"></td>` : ''}</tr>`
    } else {
  ...
  function handleExportExcel() {
    ...
    const invCol = t.payments.associatedInvoiceNumber ?? 'رقم الفاتورة المرتبطة'
    const headers = [t.payments.voucherNumber, t.date, invCol, postedLabel, t.status, t.journal.branch, t.amount, t.payments.paymentMethod, t.journal.customerOrVendor, t.payments.reference, t.payments.employeeFilter ?? 'الموظف']
    ...
    sortedRows.forEach((p) => {
      ...
      lines.push([p.number, formatDisplayDate(p.date as string), p.invoice?.number ?? '', isPosted ? (lang === 'ar' ? 'نعم' : 'Yes') : (lang === 'ar' ? 'لا' : 'No'), statusText, p.branch?.name ?? '', Number(p.amount), pmLabel === '—' ? '' : pmLabel, p.customer?.name ?? '', p.reference ?? '', empName].join(','))
    })
    lines.push('')
    ...
    a.href = url
    a.download = `receipts-report-${dateFrom}-${dateTo}.csv`
    a.click()
  ...
  return (
    <div className="px-0 py-3 space-y-3 w-full min-w-0 max-w-full">
      {/* نفس هيكل شريط فواتير المبيعات: عنوان | الفترة وتواريخ مخصصة في المنتصف | أزرار */}
              ...
              <div className="flex flex-wrap items-center gap-2 justify-center">
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <input
                    type="date"
                    value={dateFrom}
                    onChange={(e) => onReceiptDateFromChange(e.target.value)}
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <input
                    type="date"
                    value={dateTo}
                    onChange={(e) => onReceiptDateToChange(e.target.value)}
                    className="h-10 box-border border border-slate-300 rounded-lg px-2 py-0 text-sm w-[140px] min-w-[140px] bg-white leading-normal focus:ring-2 focus:ring-inset focus:ring-primary-500 focus:border-primary-500 outline-none"
                    title={labelTo}
                  />
                </div>
          ...
          <div className="relative" ref={columnsMenuRef}>
            ...
            {showColumnsMenu && (
              <div className="absolute top-full end-0 mt-1 z-50 min-w-[200px] bg-white border border-slate-200 rounded-lg shadow-lg py-2 max-h-72 overflow-y-auto">
                ...
                {RECEIPTS_REPORT_COLUMN_KEYS.map((key) => (
      ...
      <div className={`bg-white rounded-xl border border-slate-200 p-4 ${filterRowClass}`}>
        ...
        <div className={filterCellGrow}>
          <AccountSearchSelect
            ...
            onChange={(id) => {
              setCounterpartAccountIdFilter(id ?? '')
              setPage(1)
            }}
        ...
        <div className={filterCellGrow}>
          <AccountSearchSelect
            value={cashBankAccountIdFilter === '' ? null : cashBankAccountIdFilter}
            accounts={cashBankAccounts.length ? cashBankAccounts : allAccounts}
            onChange={(id) => {
              setCashBankAccountIdFilter(id ?? '')
              setPage(1)
            }}
            placeholder={t.payments.cashBankAccountFilter ?? 'حساب الصندوق/البنك'}
            className="w-full min-w-0"
          />
        </div>
            ...
            <table className="w-full text-sm table-fixed">
              <thead>
                <tr className="bg-slate-50 text-slate-600 border-b border-slate-200">
                  ...
                  {visibleColumns.invoiceNumber && <SortableTh label={t.payments.associatedInvoiceNumber ?? 'رقم الفاتورة المرتبطة'} sortKey="invoiceNumber" sortState={sort} onToggle={toggleSort} className={`${textAlign} p-0 font-medium`} />}
                  ...
                  {visibleColumns.branch && <SortableTh label={t.journal.branch} sortKey="branch" sortState={sort} onToggle={toggleSort} className={`${textAlign} p-0 font-medium`} />}
                  ...
                  {visibleColumns.customer && <SortableTh label={t.journal.customerOrVendor} sortKey="customer" sortState={sort} onToggle={toggleSort} className={`${textAlign} p-0 font-medium`} />}
                  ...
                  {visibleColumns.employee && <SortableTh label={t.payments.employeeFilter ?? 'الموظف'} sortKey="employee" sortState={sort} onToggle={toggleSort} className={`${textAlign} p-0 font-medium`} />}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200">
                {payments.length === 0 ? (
                  <tr><td colSpan={noDataColSpan} className="text-center py-12 text-slate-400">{t.payments.noReceipts}</td></tr>
                ) : (
                  sortedRows.map((p) => {
                    const isPosted = !!(p as Payment & { journal_entry_id?: number }).journal_entry_id
                    const payStatus = (p.status === 'posted' ? 'approved' : p.status) || 'approved'
                      ...
                      <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        {visibleColumns.invoiceNumber && (
                          <td className="px-4 py-3">
                            {p.invoice_id ? <Link to={`/invoices/view/${p.invoice_id}`} className="text-primary-600 hover:underline font-medium">{p.invoice?.number ?? p.invoice_id}</Link> : <span className="text-slate-400">—</span>}
                          </td>
                        )}
                        ...
                        {visibleColumns.branch && <td className="px-4 py-3 text-slate-700">{p.branch?.name ?? '—'}</td>}
                        ...
                        {visibleColumns.customer && <td className="px-4 py-3 text-slate-600">{p.customer?.name ?? '—'}</td>}
                        {visibleColumns.reference && <td className="px-4 py-3 text-slate-500 text-xs">{p.reference ?? '—'}</td>}
                        {visibleColumns.employee && <td className="px-4 py-3 text-slate-600 text-xs">{createdByDisplayName(p, '—')}</td>}
                      </tr>
                    )
                  })
                )}
