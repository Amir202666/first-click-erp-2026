...
import { createLeaveType, deleteLeaveTypeForTenant, listLeaveTypes, updateLeaveType } from '../../api/hr'
...
export default function LeaveTypesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['hr', 'leave-types', tenantId, params],
    queryFn: () => listLeaveTypes({ tenant_id: tenantId, ...params }),
    enabled: !!tenantId,
  })
  const rows: any[] = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : []

  type LtSortKey = 'name' | 'description' | 'status'
  const leaveTypeSortColumns = useMemo((): SortColumn<any, LtSortKey>[] => {
    return [
  ...
  const { sort, toggleSort, sortedRows } = useClientSort(rows, leaveTypeSortColumns, { locale })
  ...
  const createMut = useMutation({
    mutationFn: (payload: any) => createLeaveType({ tenant_id: tenantId, ...payload }),
    onSuccess: async () => {
      setCreateOpen(false)
      await qc.invalidateQueries({ queryKey: ['hr', 'leave-types', tenantId] })
    },
  })

  const updateMut = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: any }) => updateLeaveType(id, { tenant_id: tenantId, ...payload }),
    onSuccess: () => {
      setEditRow(null)
      qc.invalidateQueries({ queryKey: ['hr', 'leave-types', tenantId] })
    },
  })

  const deleteMut = useMutation({
    mutationFn: (id: number) => deleteLeaveTypeForTenant(tenantId, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['hr', 'leave-types', tenantId] }),
  })

  return (
    <HrPageShell title={(t as any).hr?.leaveTypesTitle ?? (isRtl ? 'أنواع الإجازات' : 'Leave Types')}>
            ...
            <tbody>
              ...
              {sortedRows.map((r, idx) => (
                <RowReadOnly
                  ...
                  onEdit={() => setEditRow(r)}
                  onDelete={() => {
                    if (confirm(isRtl ? 'حذف نوع الإجازة؟' : 'Delete leave type?')) deleteMut.mutate(r.id)
                  }}
      ...
      {createOpen && (
        <LeaveTypeModal
          ...
          title={isRtl ? 'إضافة نوع إجازة' : 'Add leave type'}
      ...
      {editRow && (
        <LeaveTypeModal
          ...
          title={isRtl ? 'تعديل نوع إجازة' : 'Edit leave type'}
          initial={{
            name: editRow.name ?? '',
          ...
          onDelete={() => {
            if (confirm(isRtl ? 'حذف نوع الإجازة؟' : 'Delete leave type?')) deleteMut.mutate(editRow.id)
          }}
        />
      )}
    </HrPageShell>
...
function LeaveTypeModal({
  ...
  onDelete?: () => void
  initial?: { name: string; name_en?: string | null; description_ar?: string | null; description_en?: string | null; status: 'active' | 'inactive' }
}) {
