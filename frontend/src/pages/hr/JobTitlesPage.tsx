...
import { createJobTitle, deleteJobTitleForTenant, listJobTitles, updateJobTitle } from '../../api/hr'
...
export default function JobTitlesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['hr', 'job-titles', tenantId, params],
    queryFn: () => listJobTitles({ tenant_id: tenantId, ...params }),
    enabled: !!tenantId,
  })
  const rows: any[] = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : []

  type JtSortKey = 'name' | 'description' | 'status'
  const jobTitleSortColumns = useMemo((): SortColumn<any, JtSortKey>[] => {
    return [
  ...
  const { sort, toggleSort, sortedRows } = useClientSort(rows, jobTitleSortColumns, { locale })
  ...
  const createMut = useMutation({
    mutationFn: (payload: any) => createJobTitle({ tenant_id: tenantId, ...payload }),
    onSuccess: async () => {
      setCreateOpen(false)
      await qc.invalidateQueries({ queryKey: ['hr', 'job-titles', tenantId] })
    },
  })

  const updateMut = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: any }) => updateJobTitle(id, { tenant_id: tenantId, ...payload }),
    onSuccess: () => {
      setEditRow(null)
      qc.invalidateQueries({ queryKey: ['hr', 'job-titles', tenantId] })
    },
  })

  const deleteMut = useMutation({
    mutationFn: (id: number) => deleteJobTitleForTenant(tenantId, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['hr', 'job-titles', tenantId] }),
  })

  return (
    <HrPageShell title={(t as any).hr?.jobTitlesTitle ?? (isRtl ? 'المسميات الوظيفية' : 'Job Titles')}>
            ...
            <tbody>
              ...
              {sortedRows.map((r, idx) => (
                <RowReadOnly
                  ...
                  onEdit={() => setEditRow(r)}
                  onDelete={() => {
                    if (confirm(isRtl ? 'حذف المسمى الوظيفي؟' : 'Delete job title?')) deleteMut.mutate(r.id)
                  }}
      ...
      {createOpen && (
        <JobTitleModal
          ...
          title={isRtl ? 'إضافة مسمى وظيفي' : 'Add job title'}
      ...
      {editRow && (
        <JobTitleModal
          ...
          title={isRtl ? 'تعديل مسمى وظيفي' : 'Edit job title'}
          initial={{
            name: editRow.name ?? '',
          ...
          onDelete={() => {
            if (confirm(isRtl ? 'حذف المسمى الوظيفي؟' : 'Delete job title?')) deleteMut.mutate(editRow.id)
          }}
        />
      )}
    </HrPageShell>
