...
export default function DepartmentsPage() {
  ...
  const { data: adminsData } = useQuery({
    queryKey: ['hr', 'administrations', tenantId, 'all'],
  ...
  const params = useMemo(() => {
    ...
    return p
  }, [q, status, administrationId])

  const { data, isLoading } = useQuery({
    queryKey: ['hr', 'departments', tenantId, params],
    queryFn: () => listDepartments({ tenant_id: tenantId, ...params }),
    enabled: !!tenantId,
  })
  const rows: any[] = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : []

  type DeptSortKey = 'name' | 'administration' | 'manager' | 'description' | 'status'
  const deptSortColumns = useMemo((): SortColumn<any, DeptSortKey>[] => {
    return [
      { key: 'name', type: 'string', getValue: (r) => String(r.name ?? '') },
      { key: 'administration', type: 'string', getValue: (r) => String(r.administrationRef?.name ?? r.administration?.name ?? '') },
  ...
  const { sort, toggleSort, sortedRows: sortedDeptRows } = useClientSort(rows, deptSortColumns, { locale })
  ...
  const createMut = useMutation({
    mutationFn: (payload: any) => createDepartment({ tenant_id: tenantId, ...payload }),
    onSuccess: async () => {
      setCreateOpen(false)
      await qc.invalidateQueries({ queryKey: ['hr', 'departments', tenantId] })
    },
  })

  const updateMut = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: any }) => updateDepartment(id, { tenant_id: tenantId, ...payload }),
    onSuccess: () => {
      setEditRow(null)
      qc.invalidateQueries({ queryKey: ['hr', 'departments', tenantId] })
    },
  })

  const deleteMut = useMutation({
    mutationFn: (id: number) => deleteDepartmentForTenant(tenantId, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['hr', 'departments', tenantId] }),
  })

  return (
    <HrPageShell
      title={t.hr?.departmentsTitle ?? (isRtl ? 'الأقسام' : 'Departments')}
    >
        ...
        <div className="card-padding">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
            ...
            <div className="lg:col-span-3">
              <select className="input-app" value={administrationId} onChange={(e) => setAdministrationId(e.target.value ? +e.target.value : '')}>
        ...
        <div className="table-responsive-wrap">
          <table className="table-zebra w-full text-sm">
            ...
            <tbody>
              ...
              {!isLoading && sortedDeptRows.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center text-slate-500">
                    {t.noData}
                  </td>
              ...
              {sortedDeptRows.map((r, idx) => (
                <DeptRowReadOnly
                  ...
                  onEdit={() => setEditRow(r)}
                  onDelete={() => {
                    if (confirm(isRtl ? 'حذف القسم؟' : 'Delete department?')) deleteMut.mutate(r.id)
                  }}
      ...
      {createOpen && (
        <DeptModal
          ...
          title={isRtl ? 'إضافة قسم' : 'Add department'}
      ...
      {editRow && (
        <DeptModal
          ...
          title={isRtl ? 'تعديل قسم' : 'Edit department'}
          initial={{
            name: editRow.name ?? '',
          ...
          onDelete={() => {
            if (confirm(isRtl ? 'حذف القسم؟' : 'Delete department?')) deleteMut.mutate(editRow.id)
          }}
        />
      )}
    </HrPageShell>
...
function DeptRowReadOnly({
  ...
  useEffect(() => {
    ...
    // first pass uses menuH=0 (before mount), then measure after paint
    recompute()
  ...
  const adminName = row.administration?.name ?? '—'
  ...
  return (
    <tr>
      ...
      <td className="cell-ellipsis text-left">{adminName}</td>
      ...
      <td className="text-left">
        <span
          className={`inline-flex px-2 py-1 rounded-app text-xs ${row.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}
        >
          {statusLabel}
        </span>
      </td>
      ...
      <div className="bg-white rounded-app border border-slate-200 shadow-xl w-full max-w-4xl">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm font-semibold text-slate-700">
                {isRtl ? 'الاسم بالإنجليزية' : 'Name (English)'} <span className="text-red-500">*</span>
              </label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-semibold text-slate-700">
                {isRtl ? 'الإدارة' : 'Administration'} <span className="text-red-500">*</span>
              </label>
              <select
                className="input-app mt-2"
                value={administrationId}
                onChange={(e) => setAdministrationId(e.target.value ? +e.target.value : '')}
              >
        ...
        <div className="px-4 py-3 border-t border-slate-200 flex justify-end gap-2">
          ...
          <button
            ...
            disabled={!name.trim() || (!isEdit && !administrationId) || loading}
            onClick={() =>
              onSubmit({
                name: name.trim(),
