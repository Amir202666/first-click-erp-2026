...
import type { CustomerImportRowParsed } from '../../../types/customerImport'

type Filter = 'all' | 'valid' | 'error' | 'duplicate'

interface StepPreviewProps {
  lang: 'ar' | 'en'
  rows: CustomerImportRowParsed[]
}
        ...
        <table className="min-w-full text-sm">
          <thead className="sticky top-0 bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2 text-start">{isAr ? 'اسم العميل' : 'Name'}</th>
              <th className="px-3 py-2 text-start">{isAr ? 'الهاتف' : 'Phone'}</th>
              <th className="px-3 py-2 text-start">{isAr ? 'رصيد' : 'Balance'}</th>
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r._rowIndex} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2">{r.opening_balance ?? '—'}</td>
...
export function countValidPreviewRows(rows: CustomerImportRowParsed[]): number {
