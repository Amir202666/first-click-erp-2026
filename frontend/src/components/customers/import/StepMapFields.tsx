import type { CustomerImportFieldKey, FieldMapping } from '../../../types/customerImport'
import { CUSTOMER_IMPORT_FIELDS } from '../../../types/customerImport'
...
export default function StepMapFields({ lang, mapping, onChange }: StepMapFieldsProps) {
  ...
  const updateField = (fileColumn: string, systemField: CustomerImportFieldKey | null) => {
    ...
    <div className="space-y-4">
      {!nameMapped ? (
        <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
          {isAr ? 'يجب ربط عمود بـ «اسم العميل» للمتابعة' : 'Map at least one column to Customer name'}
        </p>
      ) : null}
            ...
            {mapping.map((m) => (
              <tr key={m.fileColumn} className="border-t border-slate-100">
                ...
                <td className="px-4 py-2">
                  <select
                    value={m.systemField ?? ''}
                    onChange={(e) => updateField(m.fileColumn, (e.target.value || null) as CustomerImportFieldKey | null)}
...
export function isNameFieldMapped(mapping: FieldMapping[]): boolean {
