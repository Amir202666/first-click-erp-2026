...
import { downloadVendorImportErrorReport } from '../../../api/importVendors'
import type { VendorImportResult } from '../../../types/vendorImport'
    ...
    <div className="space-y-6 text-center">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {result.errorRows.length > 0 ? (
          <button
            type="button"
            onClick={() => downloadVendorImportErrorReport(result.errorRows)}
        ...
        >
          {isAr ? 'عرض الموردين' : 'View vendors'}
        </button>
      </div>
    </div>
  )
