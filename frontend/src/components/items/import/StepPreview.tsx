...
import type { ItemImportRowParsed } from '../../../types/itemImport'
...
interface StepPreviewProps {
  lang: 'ar' | 'en'
  rows: ItemImportRowParsed[]
  settings: ItemImportSettings
  onSettingsChange: (settings: ItemImportSettings) => void
}

export default function StepPreview({ lang, rows, settings, onSettingsChange }: StepPreviewProps) {
  const isAr = lang === 'ar'
  ...
  return (
    <div className="space-y-4">
      <StepImportSettings lang={lang} settings={settings} onChange={onSettingsChange} />
        ...
        <table className="min-w-full text-sm">
          <thead className="sticky top-0 bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2 text-start">{isAr ? 'اسم الصنف' : 'Name'}</th>
              <th className="px-3 py-2 text-start">{isAr ? 'كود' : 'Code'}</th>
              <th className="px-3 py-2 text-start">{isAr ? 'الفئة' : 'Category'}</th>
              <th className="px-3 py-2 text-start">{isAr ? 'الوحدة الأساسية' : 'Base unit'}</th>
              <th className="px-3 py-2 text-start">{isAr ? 'سعر البيع' : 'Price'}</th>
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r._rowIndex} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 font-mono text-xs">{r.category_code || r.category_name || '—'}</td>
                <td className="px-3 py-2 text-xs">{r.base_unit_symbol || r.base_unit_name || r.unit_name || '—'}</td>
                ...
                <td className="px-3 py-2">
                  ...
                  {r._errors.length > 0 ? (
                    ...
                    <span className="ms-1 text-xs text-amber-600">{isAr ? 'كود موجود' : 'Code exists'}</span>
                  ) : null}
                </td>
              </tr>
            ))}
...
export function countValidPreviewRows(rows: ItemImportRowParsed[]): number {
