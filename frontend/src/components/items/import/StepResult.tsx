...
import { downloadItemImportErrorReport } from '../../../api/importItems'
import type { ItemImportResult } from '../../../types/itemImport'
  ...
  return (
    <div className="space-y-6 text-center">
      ...
      <h2 className="text-lg font-semibold text-slate-900">
        {isAr ? 'اكتمل استيراد الأصناف' : 'Item import completed'}
      </h2>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <p className="text-xs text-slate-500">{isAr ? 'محدَّث' : 'Updated'}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-2xl font-bold text-slate-700">{result.categories_created ?? 0}</p>
          <p className="text-xs text-slate-500">{isAr ? 'فئات جديدة' : 'Categories'}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-2xl font-bold text-slate-700">{result.units_created ?? 0}</p>
          <p className="text-xs text-slate-500">{isAr ? 'وحدات جديدة' : 'Units'}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {result.errorRows.length > 0 ? (
          <button
            type="button"
            onClick={() => downloadItemImportErrorReport(result.errorRows)}
